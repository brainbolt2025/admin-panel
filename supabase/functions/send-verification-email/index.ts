...
serve(async (req) => {
  ...
  try {
    ...
    if (isTestMode) {
      ...
      console.log('âœ… Using WEB URL format (TEST MODE - same as work order emails)')
      console.log('Using port:', port)
    } else if (APP_URL) {
      // Use configured app URL
      verifyLink = `${APP_URL}/verify?token=${token}`
      linkFormat = 'WEB_URL_CONFIGURED'
      console.log('âœ… Using WEB URL format (from APP_URL configuration)')
    } else {
      // Final fallback - web URL
      verifyLink = `https://www.sycnmore.com/verify?token=${token}`
      linkFormat = 'WEB_URL_FALLBACK'
      console.log('âš ï¸ Using WEB URL format (FALLBACK)')
    }
    ...
    if (MAILGUN_API_KEY.startsWith('pubkey-')) {
      return new Response(
        JSON.stringify({
          success: false,
          error: 'You are using a Public API key (pubkey-). For sending emails, you need a Private API key. Please go to Mailgun Dashboard â†’ Settings â†’ API Keys â†’ Private API key and use that key instead.',
        }),
        {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    ...
    const htmlBody = asineEmailHtml({
      ...
      greeting: `Hi ${name},`,
      paragraphs: ['Please verify your email to activate your Property Manager account.'],
...
1. Verify MAILGUN_API_KEY is correct - copy the full key from Mailgun Dashboard â†’ Settings â†’ API Keys
2. Ensure you're using the PRIVATE API key (not the Public/Validation key)
3. Verify MAILGUN_DOMAIN matches exactly what's shown in Mailgun Dashboard â†’ Sending â†’ Domains
...
Mailgun error response: ${mailgunErrorText || mailgunResult.message || mailgunResponse.statusText || 'Forbidden'}`
    ...
    if (mailgunResult.error) {
      console.error('âš ï¸ Mailgun returned error in response:', mailgunResult.error)
...
2. Optional - web URL configuration (auto-detected in test mode):
  ...
  - In test mode (auto): uses http://localhost:8081/verify?token=xxx âœ… WORKS WITH APP
