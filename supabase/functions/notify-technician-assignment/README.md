...
This edge function sends an email notification to a technician when they are assigned to a work order.

## Purpose

When a Property Manager assigns a technician to a work order, this function:
1. Fetches the work order details
2. Retrieves the technician's email address
3. Sends a formatted email notification with work order information
...
- `APP_DEEP_LINK_SCHEME` - Custom URL scheme for mobile app (e.g., `asine://` or `oms://`)
  - If set, creates deep links like: `asine://work-order/{work_order_id}`
  - If not set, falls back to universal link
- `APP_URL` - Optional, base URL for universal links (falls back to `BASE_URL`)
- `BASE_URL` - Optional, fallback base URL
...
- **Production**: Uses production URLs from `APP_URL` or `BASE_URL` (default: `https://app.asine.app`)
...
const response = await fetch(
  'https://YOUR_PROJECT.supabase.co/functions/v1/notify-technician-assignment',
  {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
if (result.success) {
  console.log('Notification sent to:', result.technician_email)
}
```
...
- Only sends email if work order has an assigned technician
- Email is sent via Mailgun API
- **Deep Links**: The "View Work Order" button uses deep links to open the mobile app directly
  - If `APP_DEEP_LINK_SCHEME` is set (e.g., `asine://`), uses custom URL scheme: `asine://work-order/{id}`
  - Otherwise, uses universal link: `{APP_URL}/work-order/{id}`
  - Make sure your mobile app is configured to handle these deep link patterns
