...
serve(async (req) => {
  ...
  try {
    ...
    if (!TENANT_APP_URL && !TENANT_APP_DEEP_LINK_SCHEME) {
    ...
    const workOrderLink = TENANT_APP_DEEP_LINK_SCHEME
      ? `${TENANT_APP_DEEP_LINK_SCHEME}work-order/${work_order_id}`
      : `${TENANT_APP_URL}/work-order/${work_order_id}`
